...
def build_matrix_image(thumbnail_bytes, filename="current_album.png"): #funciton uses the thumbnail bytes which is raw image from SMTC and were going to put it into this file "current_album.png"
    ...
    img_square = img.resize((64, 64)) # resize the image to 32 x 32 regardless of size now 64 64 

    canvas = Image.new("RGB", (64, 64), (0, 0, 0)) # Im taking the orignal matrix and making it black to start old version had a y value of 32 not 64
...
def send_image_to_matrix(canvas): # Taking one input that being THE Resized and Adjusted image from function "build_matrix_image"
    ...
    for y in range(64): #This loop must be the same order ie Y first then X same as the ardunio code as its reading in this order rather then here where were going thru and sending those values in that order --- new matrix 64 time
